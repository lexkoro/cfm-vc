...
class Wrapper(nn.Module):
    def __init__(self, vector_field_net, mask, mu, spk, guidance_scale):
    ...
    def forward(self, t, x, args):
        ...
        dphi_dt = self.net(x, self.mask, self.mu, t, self.spk)

        if self.guidance_scale > 0.0:
            mu_avg = self.mu.mean(2, keepdims=True).expand_as(self.mu)
            dphi_avg = self.net(x, self.mask, mu_avg, t, self.spk)
...
class FM(nn.Module):
    ...
    def ode_wrapper(self, mask, mu, spk, guidance_scale=0.0):
        # self.estimator receives x, mask, mu, t, spk as arguments
        return Wrapper(self.estimator, mask, mu, spk, guidance_scale)
    ...
    def inference(
        ...
        neural_ode = NeuralODE(
            self.ode_wrapper(mask, mu, spk, guidance_scale),
            solver=solver,
            sensitivity="adjoint",
            atol=1e-4,
            rtol=1e-4,
