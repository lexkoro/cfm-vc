...
class LearnableUpsamplingLayer(torch.nn.Module):
    def __init__(
        ...
        self.proj_in = nn.Sequential(
            OrderedDict(
                [("linear", nn.Linear(in_channels, in_channels)), ("swish", nn.SiLU())]
            )
        )

        self.conv_norm_swish = nn.Sequential(
            OrderedDict(
                [
                    (
                        "conv_ln",
                        ConvLN(
                            in_channels, conv_hidden_channels, kernel_size=3, padding=1
                        ),
                    ),
                    ("swish_2", nn.SiLU()),
                ]
        ...
        conv_hidden_channels = conv_hidden_channels + 2
        self.W_proj = nn.Sequential(
            OrderedDict(
                [
                    ("linear_1", nn.Linear(conv_hidden_channels, conv_hidden_channels)),
                    ("swish", nn.SiLU()),
                    ("linear_2", nn.Linear(conv_hidden_channels, attn_dims)),
        ...
        self.C_proj = nn.Sequential(
            OrderedDict(
                [
                    ("linear_1", nn.Linear(conv_hidden_channels, conv_hidden_channels)),
                    ("swish_1", nn.SiLU()),
                    ("linear_2", nn.Linear(conv_hidden_channels, content_dims)),
                    ("swish_2", nn.SiLU()),
                ]
            )
        )
    ...
    def forward(self, H, d, src_mask):
        ...
        """
        if len(src_mask.size()) == 2:
            src_mask = src_mask.unsqueeze(1)

        S, E, mel_mask = self.token_boundary_grid(d, src_mask)  # [b, t_s, t_t, 1]
        b, _, t_mel = src_mask.shape[0], src_mask.shape[-1], mel_mask.shape[-1]
    ...
    def token_boundary_grid(self, dur, src_mask):
        ...
        token_boundary_mask = (
            torch.unsqueeze(src_mask, 2) * torch.unsqueeze(~mel_mask, -1)
        ).squeeze()
